...
$course=$_POST['course'];
$sem=$_POST['sem'];
$sql = "SELECT c.credits FROM course c JOIN courseenrolls s ON c.id = s.course WHERE s.studentRegno = '$studentregno';";
$result = mysqli_query($con, $sql);
$sum_of_credits = 0;
while ($row = mysqli_fetch_assoc($result)) {
    $sum_of_credits += $row['credits'];
}
$sql = "SELECT credits FROM course WHERE id = $course";
$result = mysqli_query($con, $sql);
$row = mysqli_fetch_assoc($result);
$new_credits = $row['credits'];
$query = "SELECT maxcredits FROM department WHERE id='$dept'";
$result = mysqli_query($con, $query);
$row = mysqli_fetch_assoc($result);
$maxcr = $row['maxcredits'];
if (($sum_of_credits + $new_credits) > $maxcr) {
  echo "You cannot enroll in this course as it would exceed your allotted course credits.";
}
else
{
...
echo '<script>alert("Error : Not Enrolled")</script>';
echo '<script>window.location.href=enroll.php</script>';
}
}
}
...
{
?>
<option value="<?php echo htmlentities($row['id']);?>"><?php echo htmlentities($row['department']);?> (<?php echo htmlentities($row['maxcredits']); ?> credits allowed)</option></option>
<?php } ?>
...
while ($row = mysqli_fetch_array($sql)) {
?>
<option value="<?php echo htmlentities($row['id']); ?>"><?php echo htmlentities($row['courseName']); ?> (<?php echo htmlentities($row['credits']); ?> credits)</option>
<?php } 
?>
    </select> 
